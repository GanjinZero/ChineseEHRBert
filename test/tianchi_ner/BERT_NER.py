...
flags.DEFINE_integer("doc_stride", 128, "length of stride to process long document > bert max length")
...
class DataProcessor(object):
    ...
    def _read_data(cls,input_file):
        ...
        for l, w in lines:
            ...
            max_tokens_for_doc = FLAGS.max_seq_length - 1
            len_tokens = len(word_list)
            doc_spans = []
            start_offset = 0
            while start_offset < len_tokens:
                length = len_tokens - start_offset
                if length > max_tokens_for_doc:
                    length = max_tokens_for_doc
                doc_spans.append([start_offset, start_offset + length])
                if start_offset + length == len_tokens:
                    break
                start_offset += min(length, FLAGS.doc_stride)
            
            for span in doc_spans:
                l_cut = label_list[span[0]:span[1]]
                w_cut = word_list[span[0]:span[1]]
                new_lines.append((' '.join(l_cut), ' '.join(w_cut)))
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        """
        marks = ["O"]
        ner_list = ['Disease', 'Reason', 'Symptom', 'Test', 'Test_Value',
            'Drug', 'Frequency', 'Amount', 'Method', 'Treatment',
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        with open(output_eval_file,"w") as wf:
            ...
            p,r,f = metrics.calculate(confusion_matrix,len(label_list[2:]))
            logging.info("***********************************************")
