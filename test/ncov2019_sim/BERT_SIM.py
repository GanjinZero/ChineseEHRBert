...
flags.DEFINE_integer("iterations_per_loop", 100000,
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature, tokens, label = convert_single_example(ex_index, example, label_list,
...
def main(_):
    ...
    if FLAGS.do_eval:
        ...
        output_eval_file = os.path.join(FLAGS.output_dir, "eval_results.csv")
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            pred_labels = []
            for tokens, true_label, prediction in zip(batch_tokens, batch_labels, results):
                probabilities = prediction["probabilities"]
                probabilities = [float(prob) for prob in probabilities]
                pred_prob = round(probabilities[1], 3)
                pred_label = probabilities.index(max(probabilities))
                writer.write(f'{tokens}\t{true_label}\t{pred_prob}\t{pred_label}\n')
                pred_labels.append(pred_label)
        
        summary_eval_file = os.path.join(FLAGS.output_dir, "eval_summary.txt")
        with tf.gfile.GFile(summary_eval_file, "w") as writer:
            assert len(batch_labels) == len(pred_labels)
            report = classification_report(batch_labels, pred_labels, digits=4)
            writer.write(report)
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            for tokens, prediction in zip(batch_tokens, results):
                probabilities = prediction["probabilities"]
                probabilities = [float(prob) for prob in probabilities]
                pred_prob = round(probabilities[1], 3)
                pred_label = probabilities.index(max(probabilities))
                writer.write(f'{tokens}\t{pred_prob}\t{pred_label}\n')
